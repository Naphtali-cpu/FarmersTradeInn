...
import com.naph.startup.config.MpesaConfiguration;
import com.naph.startup.dto.mpesa.*;
import com.naph.startup.utils.Constants;
import com.naph.startup.utils.HelperUtility;
...
import static com.naph.startup.utils.Constants.*;
...
public class DarajaApiImpl implements DarajaApi {
    ...
    public RegisterUrlResponse registerUrl() {
        ...
        try {
            ...
            assert response.body() != null;
            log.info(String.format("Here is our data -> %s", response.body()));
...
//    We'll perform an STK push here for the customer to pay. Basically the user will have to input their phone number and amount to pay and they will get a promt to enter their pin on their phone
    ...
    public StkPushSyncResponse performStkPushTransaction(InternalStkPushRequest internalStkPushRequest) {
        ...
        log.info(String.format("Here is our access token data -> %s", accessTokenResponse.getAccessToken()));
        ...
        try {
            Response response = okHttpClient.newCall(request).execute();
            assert response.body() != null;
            log.info(String.format("Here is our stk data -> %s", response.body()));
            return objectMapper.readValue(response.body().toString(), StkPushSyncResponse.class);
        } catch (IOException e) {
